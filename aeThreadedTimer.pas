Unit aeThreadedTimer;
//ae's uthreadedTimer Unit
Interface
...
Begin
  ...
  Begin
    // Wait for the Enabled and Cancelled flags for an infinite amount of time. If not Object_0 (Enabled flag) was
    // signaled (thus, the timer thread was cancelled) exit the thread immediately.
    If WaitForMultipleObjects(2, @_events[0], False, INFINITE) <> WAIT_OBJECT_0 Then
      Exit;
...
Begin
 Result := Not Self.Terminated And (WaitForSingleObject(_events[0], 0) = WAIT_OBJECT_0);
End;
...
Function TThreadedTimer.GetInterval: Integer;
Begin
 Result := TTimerThread(_thread).Interval;
End;

Procedure TThreadedTimer.ThreadTimer;
Begin
 If Assigned(_ontimer) Then
   _ontimer(Self);
End;

Procedure TThreadedTimer.SetEnabled(Const inEnabled: Boolean);
Begin
 _enabled := inEnabled;
...
Procedure TThreadedTimer.SetInterval(Const inInterval: Integer);
Begin
 TTimerThread(_thread).Interval := inInterval;
End;

Procedure TThreadedTimer.SetOnTimer(Const inOnTimer: TNotifyEvent);
Begin
 _ontimer := inOnTimer;
...
procedure TThreadedTimer.Start;
begin
  Enabled := True;
end;

procedure TThreadedTimer.Step;
begin
  Enabled := False;
  OnTimer(self);
  //Synchronize(_ontimer);
